...
void Internal::pure_literal_elimination () {
  cout << "HERE" << endl;
  bool something_propagated = false;
  do {
    something_propagated = false;
    ...
    for (auto lit: lits) {
      ...
      // cout << "Lit: " << lit << " Pos: " << noccs(lit) << " Neg: " << noccs(-lit) << endl;
      ...
      if (pure_literal) {
        cout << "c PURE " << pure_literal << endl;
        search_assign ((int) pure_literal, (Clause*) (0));
        if (!propagate()) return;
        if (unsat)
          return;
        garbage_collection (); // remove propagated literals
        something_propagated = true;
        break;
      }
    }
  } while (something_propagated);
}
...
int Internal::solve (bool preprocess_only) {
  ...
  if (opts.ple) pure_literal_elimination ();
